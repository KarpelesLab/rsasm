...
impl Assembler {
    ...
    fn plain_fixup_value(
        ...
        if let (Some(p), Some(m)) = (v.plus, v.minus) {
            ...
            if ps.is_some() && (ps == ms || !self.options.relocatable) {
                // Unless the two may be moved apart: on x86-64, a Mach-O
                // object keeps a difference between atoms for the linker.
    ...
    pub(crate) fn macho_object(&self) -> bool {
        self.options.format == Format::MachO && self.options.relocatable
    }
    ...
    fn macho_relocation(
        ...
        let cpu = crate::output::macho::Cpu::for_arch(out);
        let arm64 = cpu == Some(crate::output::macho::Cpu::Arm64);
        let page = matches!(
            desc.class,
        ...
        if let Some(cpu) = cpu
            && crate::output::macho::reloc_type(cpu, &r).is_none()
        {
            let msg = match (desc.class, desc.pcrel) {
                (crate::reloc::RelocClass::SignExtended, _) => {
                ...
                (class, pcrel) => {
                    let what = match (class, pcrel) {
                        (crate::reloc::RelocClass::Plain, true) => "PC-relative ",
                        (crate::reloc::RelocClass::Plain, false) => "",
                        (crate::reloc::RelocClass::Branch, _) => "branch ",
                        _ => "GOT or page ",
                    };
                    format!(

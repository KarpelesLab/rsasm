...
fn select<'d>(
    ...
    for def in defs {
        ...
        // A suffix of the mode's own width, as in 32-bit `calll foo`, still
        // names the relative branch, which has no operand size of its own.
        let relative = def.ops.first().is_some_and(|o| matches!(o, Op::Rel(_)));

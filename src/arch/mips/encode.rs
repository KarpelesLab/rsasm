...
impl Args<'_> {
    ...
    /// `wide` is the definition's [`Def::wide_fprs`], which says whether this
    /// operand holds a 64-bit value and so names a register pair; see
    /// [`super::abi`].
    pub fn fpr(&self, cx: &mut AsmCtx<'_>, i: usize, wide: u8) -> Option<Reg> {
        let o = self.at(cx, i)?;
        match o.fpr() {
            Some(r) => {
                super::abi::mark_fpr(cx.state, r, wide & (1 << i) != 0);
                Some(r)
            }
            None => {
                cx.error(
...
pub fn encode(
    ...
    match def.form {
        ...
        Form::FtMem => {
            a.arity(cx, 2)?;
            let t = a.fpr(cx, 0, def.wide_fprs)?;
            let m = a.mem(cx, 1)?;
            place_disp(cx, &mut w, base | rt(t.num) | rs(m.base.num), m.disp)?;
        }
        Form::RsRtOff => {
        ...
        Form::FdFsFt => {
            a.arity(cx, 3)?;
            let (fd, fs, ft) = (
                a.fpr(cx, 0, def.wide_fprs)?,
                a.fpr(cx, 1, def.wide_fprs)?,
                a.fpr(cx, 2, def.wide_fprs)?,
            );
            w.push(base | sa(fd.num as u32) | rd(fs.num) | rt(ft.num));
        }
        Form::FdFs => {
            a.arity(cx, 2)?;
            let (fd, fs) = (a.fpr(cx, 0, def.wide_fprs)?, a.fpr(cx, 1, def.wide_fprs)?);
            w.push(base | sa(fd.num as u32) | rd(fs.num));
        }
        Form::FsFt => {
            a.arity(cx, 2)?;
            let (fs, ft) = (a.fpr(cx, 0, def.wide_fprs)?, a.fpr(cx, 1, def.wide_fprs)?);
            w.push(base | rd(fs.num) | rt(ft.num));
        }
        Form::RtFs => {
            a.arity(cx, 2)?;
            let t = a.gpr(cx, 0)?;
            let fs = a.fpr(cx, 1, def.wide_fprs)?;

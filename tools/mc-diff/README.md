...
`<arch>-*-words.txt` is one instruction per line too, for a fixed-width
instruction set and a corpus large enough for that to matter: its lines are
assembled 200 at a time, and only a batch whose bytes differ is gone through
a line at a time to name the line. Each line counts as a case either way.
`aarch64-simd-words.txt` and `aarch64-sve-words.txt` are generated by
`tools/tables/aarch64.py` along with the encoding table they check, which
is why they are large: every form of every instruction, at the ends of its
ranges and somewhere inside them. `aarch64-sys-words.txt` is generated by
`tools/tables/aarch64-sys.py` the same way, from GNU as's system-register
tables; the names llvm-mc does not know go to `tools/xas-diff/aarch64.txt`
instead.

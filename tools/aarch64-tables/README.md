# The AArch64 SIMD, floating-point and SVE table

`src/arch/aarch64/table_data.rs` and `table_names.rs` are generated by
`gen.py` from llvm-mc 22, and the corpora `tools/mc-diff/aarch64-simd-words.txt`
and `aarch64-sve-words.txt` with them. No encoding in them was written by hand.

```console
$ tools/aarch64-tables/gen.py --jobs 32                 # about ten minutes
$ tools/aarch64-tables/gen.py --only '^fmov$' --dry-run --dump forms.txt
$ tools/aarch64-tables/probe.py 'sshr v0.8b, v1.8b, #3'  # one line, measured
```
...
`ldapur d0, [x1]`). SME beyond `smstart`/`smstop` and `zero {za}` is left out.
...
- an immediate the word holds a function of is the bitmask immediate, the
  8-bit floating-point immediate or a byte mask, whichever explains most
  values;
...
agreeing with the model; and where that still leaves a choice, the values its
disassembler prints back decide: `scvtf s0, w0, #0` encodes `#32`, so the
range is `#1`..`#32`. And one printed shape can be several encodings: `mov
...
element's bits (`mov z0.h, #0xfff0` for `#-16`) is marked to wrap, once
llvm-mc has been seen to.
...
- `gen.py` sweeps, fits, checks and writes the table and corpora.
- `probe.py` fits one line and prints what it measured, for looking into a
  form.

...
rsasm [options] <input.s>...
  ...
  -a, --arch <name>  target architecture (default: the host, if supported),
                     ...
                     Mach-O output
  -f, --format <fmt> output format: elf (default), elf32, elf64, macho or bin
...
`tools/macho-diff/run.sh` compares 357 cases, single statements and whole
programs in Clang's style, against llvm-mc 22: every header and load command,
section, symbol and relocation matches, and each of the 334 objects both
assemblers write is identical byte for byte; the other 23 cases are refused by
both. Three differences remain, and the corpus leaves them out:
...
- `tools/macho-diff/run.sh` for [Mach-O objects](#mach-o-objects), against
  ...
  and relocations as `llvm-readobj` reads them. 357 of 357 match, and every
  object both write is also identical byte for byte.

...
`m68k`, `superh`, `rx`, `rl78`, `v850`, `k78` and `msp430`:
...
- `tools/xas-diff/run.sh` against cross GNU as 2.47 for m68k, SuperH, RX, RL78,
  ...
  against GNU as, the reference for literal pools and interworking.
  `tools/oracles/build.sh` builds the references from checksum-pinned
  sources. 13,807 of 13,807 match across twenty-eight variants.
...
- `tools/dwarf-diff/run.sh` for [debug information](#debug-information),
  ...
  Clang. 1,086 of 1,086 match across twenty-three target variants.

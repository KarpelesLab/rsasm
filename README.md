...
- `tools/mc-diff/run.sh` against llvm-mc 22, for x86 and the targets LLVM
  supports. 38,922 of 38,922 match across twenty-one target variants. For RISC-V
...
- `tools/xas-diff/run.sh` against cross GNU as 2.47 for m68k (for each CPU
  ...
  from checksum-pinned sources. 25,604 of 25,604 match across fifty-seven
  variants.
...
- `tools/link-diff/run.sh` against a link of a *whole program*: two or three
  ...
  turns into initial exec, the MIPS models that need no GOT
